...
function normalizePlayer(player: PlayerItem): PlayerItem {
  return {
    ...
    jersey_number:
      player.jersey_number === null ? null : toNumber(player.jersey_number),
...
function normalizeAdminUser(user: AdminUserItem): AdminUserItem {
  return {
    ...
    jersey_number:
      user.jersey_number === null ? null : toNumber(user.jersey_number),
  };
}
...
function normalizeMatch(match: MatchItem): MatchItem {
  return {
    ...
    participant_count:
      match.participant_count === undefined
        ? undefined
        : toNumber(match.participant_count, 0),
  };
}
...
function normalizeGlobalStats(stats: GlobalStats): GlobalStats {
  return {
    teamOverview: {
      team_overall_avg: clampPercentage(
        stats.teamOverview?.team_overall_avg,
        0,
      ),
      team_reception_avg: clampPercentage(
        stats.teamOverview?.team_reception_avg,
        0,
      ),
      team_serve_avg: clampPercentage(stats.teamOverview?.team_serve_avg, 0),
      team_defense_avg: clampPercentage(
        stats.teamOverview?.team_defense_avg,
        0,
      ),
      ...
      team_setting_avg: clampPercentage(
        stats.teamOverview?.team_setting_avg,
        0,
      ),
      team_attack_points_per_set_avg: toNumber(
        stats.teamOverview?.team_attack_points_per_set_avg,
        0,
      ),
  ...
  return {
    ...
    category_id:
      transaction.category_id === null
        ? null
        : toNumber(transaction.category_id),
...
function normalizeFinanceDebtPayment(
  payment: FinanceDebtPayment,
): FinanceDebtPayment {
  return {
    ...payment,
...
function normalizeCalendarEvent(event: CalendarEvent): CalendarEvent {
  return {
    ...
    fecha_fin_serie: event.fecha_fin_serie
      ? toDateInput(event.fecha_fin_serie)
      : null,
    ...
    instances: (event.instances ?? []).map((instance) => ({
      ...
      my_response: instance.my_response
        ? {
            ...
            respondido_en: instance.my_response.respondido_en
              ? toDateTimeInput(instance.my_response.respondido_en)
              : null,
          }
...
function normalizePlayerFinanceDebtSummary(
...
export const api = {
  ...
  me: (token: string) =>
    request<{ user: LoginResponse["user"] }>("/auth/me", {}, token),
  ...
  getMatches: async (token: string) => {
    const response = await request<{ matches: MatchItem[] }>(
      "/matches",
      {},
      token,
    );
  ...
  getCalendar: async (token: string) => {
    const response = await request<{ events: CalendarEvent[] }>(
      "/calendar",
      {},
      token,
    );
  ...
  saveCalendarAttendance: (
    token: string,
    instanceId: number,
    payload: CalendarAttendancePayload,
  ) =>
    request<{ message: string }>(
  ...
  getMatchRatings: async (token: string, matchId: number) => {
    const response = await request<{ ratings: MatchRatingRow[] }>(
      `/matches/${matchId}/ratings`,
      {},
      token,
    );
  ...
  getUsers: async (token: string) => {
    const response = await request<{ users: AdminUserItem[] }>(
      "/users",
      {},
      token,
    );
  ...
  getMyProfile: async (token: string) => {
    const response = await request<{ player: PlayerItem }>(
      "/players/me",
      {},
      token,
    );
  ...
  getPlayerStats: async (token: string, playerId: number) => {
    const response = await request<PlayerStatsResponse>(
  ...
  getFinanceOverview: async (token: string) => {
    const response = await request<{ summary: FinanceOverview }>(
      "/finance/overview",
      {},
      token,
    );
  ...
  getFinanceCategories: async (token: string, type?: FinanceType) => {
    ...
    const response = await request<{ categories: FinanceCategory[] }>(
  ...
  createFinanceCategory: (
    token: string,
    payload: { name: string; type: FinanceType },
  ) =>
    request<{ id: number; message: string }>(
      "/finance/categories",
      {
  ...
  getFinanceTransactions: async (
    token: string,
    filters?: { type?: FinanceType; from?: string; to?: string },
  ) => {
