      ...
      <article className="card xl:col-span-2">
        ...
        {editingMatchId ? (
          <p className="mt-1 text-sm text-amber-500">
            Modo edicion activo para partido #{editingMatchId}
          </p>
        ) : null}
        <form className="mt-4 space-y-3" onSubmit={onCreateMatch}>
          ...
          <input
            ...
            onChange={(event) =>
              onMatchFormChange((s) => ({ ...s, opponent: event.target.value }))
            }
            required
          />
          <input
          ...
          <input
            ...
            onChange={(event) =>
              onMatchFormChange((s) => ({ ...s, location: event.target.value }))
            }
          />
          <textarea
            ...
            onChange={(event) =>
              onMatchFormChange((s) => ({ ...s, notes: event.target.value }))
            }
          />
      ...
      <article className="card xl:col-span-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <select
            ...
            onChange={(event) =>
              onSelectedMatchChange(Number(event.target.value))
            }
          >
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <input
            ...
            onChange={(event) =>
              onEvaluationPlayerSearchTermChange(event.target.value)
            }
          />
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Mostrando {filteredEvaluationPlayers.length} de {players.length}{" "}
            jugadores
          </p>
        </div>
        ...
        <div className="mt-4 grid max-h-[34rem] gap-4 overflow-y-auto pr-1">
          {filteredEvaluationPlayers.map((player) => {
            ...
            const playerRating =
              ...
              createDefaultRating(player.player_id);
              ...
              <div key={player.player_id} className="card p-3">
                ...
                {activePlayer ? (
                  ...
                  <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">
                    Selecciona este jugador para evaluarlo.
                  </p>
                )}
              </div>
            );
