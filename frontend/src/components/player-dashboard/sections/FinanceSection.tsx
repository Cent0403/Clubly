...
export function FinanceSection({
  ...
  const [debtFilterStatus, setDebtFilterStatus] = useState<
    "ALL" | "pending" | "partially_paid" | "paid"
  >("ALL");
  ...
  const filteredUpcomingDebts = useMemo(() => {
    ...
    return upcomingDebts.filter((debt) =>
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter((payment) => {
      const fromMatches =
        !paymentFromDate || payment.payment_date >= paymentFromDate;
    ...
    <section className={active ? "grid gap-6 xl:grid-cols-5" : "hidden"}>
      <article className="card xl:col-span-2">
        ...
        <div className="mt-4 space-y-2 text-sm">
          <p>
            <span className="font-semibold">Total adeudado:</span>{" "}
            {formatMoney(summary?.totalDue ?? 0)}
          </p>
          <p>
          ...
          <p>
            <span className="font-semibold">Pendiente por pagar:</span>{" "}
            {formatMoney(summary?.totalPending ?? 0)}
          </p>
          <p>
            <span className="font-semibold">Deudas activas:</span>{" "}
            {summary?.pendingCount ?? 0}
          </p>
          <p>
            <span className="font-semibold">Pagos futuros:</span>{" "}
            {summary?.upcomingCount ?? 0}
          </p>
          <p>
            <span className="font-semibold">Próximo vencimiento:</span>{" "}
            {summary?.nextDueDate ?? "Sin fecha"}
          </p>
        </div>
      </article>
      ...
      <article className="card xl:col-span-3">
        ...
        <div className="mt-3 max-h-40 space-y-2 overflow-y-auto pr-1">
          {filteredUpcomingDebts.map((debt) => (
            <div key={debt.id} className="card p-2 text-sm">
              <div className="flex items-center justify-between gap-2">
                <p className="font-semibold">
                  {debt.description || "Deuda registrada"}
                </p>
                <span className="font-semibold text-amber-500">
                  {debt.due_date ?? "Sin fecha"}
                </span>
              </div>
              <p className="text-xs text-slate-600 dark:text-slate-300">
                Pendiente: {formatMoney(debt.amount_pending)}
              </p>
            </div>
          ))}
            ...
            <div key={debt.id} className="card p-2 text-sm">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  {getDebtStateLabel(debt.status)}
                </span>
              </div>
              <p className="text-xs text-slate-600 dark:text-slate-300">
                Total: {formatMoney(debt.amount_due)} | Pagado:{" "}
                {formatMoney(debt.amount_paid)} | Pendiente:{" "}
        ...
        <div className="mt-3 max-h-40 space-y-2 overflow-y-auto pr-1">
          {filteredPayments.map((payment) => (
            <div key={payment.id} className="card p-2 text-sm">
              <p className="font-semibold">
